...
static zval *__php_apn_payload_array(apn_payload_ref payload TSRMLS_DC);
...
#define PHP_APN_PAYLOAD_FETCH_RESOURCE(__res, __res_id, __rsrc_type, __ret_value) \
    do { \
        __res = (apn_payload_ref) zend_list_find(__res_id, &__rsrc_type); \
...
{ \
    ...
    if(IS_STRING == Z_TYPE_PP(__option_value) && 0 == Z_STRLEN_PP(__option_value)) { \
...
enum __php_apn_payload_options_code {
    ...
    APN_PAYLOAD_OPTION_PRIORITY,
    APN_PAYLOAD_OPTION_CATEGORY
};
...
static __php_apn_option __php_apn_payload_options[] = {
    ...
    {"priority", APN_PAYLOAD_OPTION_PRIORITY},
    {"category", APN_PAYLOAD_OPTION_CATEGORY}
};

ZEND_BEGIN_ARG_INFO_EX(arginfo_apn_init, 0, 0, 0)
...
ZEND_BEGIN_ARG_INFO_EX(arginfo_apn_payload_set_priority, 0, 0, 2)
ZEND_ARG_INFO(0, payload)
ZEND_ARG_INFO(0, priority)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_apn_payload_set_category, 0, 0, 2)
ZEND_ARG_INFO(0, payload)
...
const zend_function_entry apn_functions[] = {
    PHP_FE(apn_init, arginfo_apn_init)
    ...
    PHP_FE(apn_payload_set_category, arginfo_apn_payload_set_category)
    PHP_FE(apn_payload_set_priority, arginfo_apn_payload_set_priority)
...
static int __php_apn_get_option_code(const char *option) {
    ...
    for (i = 0; i < array_size; i++) {
        if (0 == strcasecmp(option, __php_apn_options[i].option)) {
...
static int __php_apn_payload_get_option_code(const char *option) {
    ...
    for (i = 0; i < array_size; i++) {
        if (0 == strcasecmp(option, __php_apn_payload_options[i].option)) {
...
static void __php_apn_payload_dtor(zend_rsrc_list_entry *rsrc TSRMLS_DC) {
    apn_payload_ref apn_payload = (apn_payload_ref) rsrc->ptr;
    if (apn_payload) {
...
STD_PHP_INI_ENTRY("apn.private_key", "", PHP_INI_ALL, OnUpdateString, apn_private_key, zend_apn_globals, apn_globals)
STD_PHP_INI_ENTRY("apn.private_key_pass", "", PHP_INI_ALL, OnUpdateString, apn_private_key_pass, zend_apn_globals, apn_globals)
PHP_INI_END()
...
PHP_MINIT_FUNCTION(apn) {
    ...
    REGISTER_LONG_CONSTANT("APN_NOTIFICATION_PRIORITY_DEFAULT", APN_NOTIFICATION_PRIORITY_DEFAULT, CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("APN_NOTIFICATION_PRIORITY_HIGH", APN_NOTIFICATION_PRIORITY_HIGH, CONST_CS | CONST_PERSISTENT);
    ...
    ret = apn_library_init();
    if (APN_ERROR == ret) {
        php_error_docref(NULL TSRMLS_CC, E_ERROR, "unable to initialize capn library");
        return FAILURE;
    }
    return SUCCESS;
}
...
PHP_FUNCTION(apn_init) {
    ...
    char *private_key = PHP_APN_G(apn_private_key);
    char *private_key_pass = PHP_APN_G(apn_private_key_pass);
    char *crt = PHP_APN_G(apn_certificate);
    char *error = NULL;

    int private_key_len = crt ? strlen(private_key) : 0;
    int private_key_pass_len = private_key_pass ? strlen(private_key_pass) : 0;
    int crt_len = crt ? strlen(crt) : 0;

    if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "|sss", &crt, &crt_len, &private_key, &private_key_len, &private_key_pass, &private_key_pass_len)) {
        return;
    }

    apn_ctx = apn_init(crt, private_key, private_key_pass);
    if (!apn_ctx) {
        printf("error\n");
        error = apn_strerror(errno);
        php_error_docref(NULL TSRMLS_CC, E_ERROR, "unable to initialize: %s (%d)", error, errno);
        free(error);
        RETURN_NULL();
    }
...
PHP_FUNCTION(apn_set_certificate) {
    ...
    if (APN_ERROR == apn_set_certificate(apn_ctx, value)) {
        error = apn_strerror(errno);
        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set SSL certificate: %s (%d)", error, errno);
        free(error);
        RETURN_FALSE
    }
    RETURN_TRUE
...
PHP_FUNCTION(apn_set_private_key) {
    ...
    if (APN_ERROR == apn_set_private_key(apn_ctx, key, key_pass)) {
        error = apn_strerror(errno);
        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set SSL private key: %s (%d)", error, errno);
        free(error);
        RETURN_FALSE
    }
    RETURN_TRUE
...
static uint8_t __php_apn_add_token(apn_ctx_ref apn_ctx, const char *token, uint32_t token_length TSRMLS_DC) {
    ...
    if (APN_ERROR == apn_add_token(apn_ctx, token)) {
        error = apn_strerror(errno);
        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to add device token '%s': %s (%d)", token, error, errno);
        free(error);
        return 1;
    }
    return 0;
...
static uint8_t __php_apn_add_tokens(apn_ctx_ref apn_ctx TSRMLS_DC, zval *tokens) {
    ...
    for (zend_hash_internal_pointer_reset_ex(Z_ARRVAL_P(tokens), &pointer_position);
        ...
        if (SUCCESS == zend_hash_get_current_data_ex(Z_ARRVAL_P(tokens), (void**) &array_item, &pointer_position)) {
            if (IS_STRING != Z_TYPE_PP(array_item)) {
...
PHP_FUNCTION(apn_set_array) {
    ...
    for (zend_hash_internal_pointer_reset_ex(Z_ARRVAL_P(array), &pointer_position);
        ...
        if (HASH_KEY_IS_STRING == array_key_type) {
        ...
        if (SUCCESS == zend_hash_get_current_data_ex(Z_ARRVAL_P(array), (void**) &array_item, &pointer_position)) {
            switch (flag) {
                case APN_OPTION_CERTIFIACE:
                    ...
                    if (APN_ERROR == apn_set_certificate(apn_ctx, Z_STRVAL_PP(array_item))) {
                        error = apn_strerror(errno);
                        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set certificate: %s (%d)", error, errno);
                        free(error);
                        RETURN_FALSE
                    }
                    break;
                ...
                case APN_OPTION_MODE:
                    if (IS_LONG != Z_TYPE_PP(array_item)) {
                        ...
                        apn_set_mode(apn_ctx, (uint8_t) Z_LVAL_PP(array_item));
                    }
                    break;
                case APN_OPTION_TOKENS:
                    if (IS_ARRAY == Z_TYPE_PP(array_item)) {
                        if (__php_apn_add_tokens(apn_ctx TSRMLS_CC, *array_item)) {
                            RETURN_FALSE
                        }
                    } else if (IS_STRING == Z_TYPE_PP(array_item)) {
                        if (__php_apn_add_token(apn_ctx, Z_STRVAL_PP(array_item), Z_STRLEN_PP(array_item) TSRMLS_CC)) {
                            RETURN_FALSE
                        }
                    } else {
    ...
    if (private_key) {
        if (APN_ERROR == apn_set_private_key(apn_ctx, private_key, private_key_pass)) {
            error = apn_strerror(errno);
            php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set private key: %s (%d)", error, errno);
            free(error);
            RETURN_FALSE
        }
        if (private_key_pass) {
...
PHP_FUNCTION(apn_send) {
    ...
    if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "rr|zzz", &res, &payload_res, &ref_error, &ref_errcode)) {
        return;
    }
    ...
    if (APN_ERROR == apn_send(apn_ctx, payload)) {
        error = apn_strerror(errno);
        ZVAL_STRING(ref_error, error, 1);
        ZVAL_LONG(ref_errcode, errno);
        free(error);
...
static uint8_t __php_apn_payload_add_token(apn_payload_ref payload_ctx, const char *token, uint32_t token_length TSRMLS_DC) {
    char *error = NULL;
    if (0 == token_length) {
        php_error_docref(NULL TSRMLS_CC, E_WARNING, "device token is not specified");
        return 1;
    }
    if (APN_ERROR == apn_payload_add_token(payload_ctx, token)) {
        error = apn_strerror(errno);
        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to add device token '%s': %s (%d)", token, error, errno);
        free(error);
        return 1;
...
static uint8_t __php_apn_payload_add_tokens(apn_payload_ref payload_ctx TSRMLS_DC, zval *tokens) {
    HashPosition pointer_position = NULL;
    int array_key_type = 0;
    char *array_strkey = NULL;
    unsigned int array_strkey_len = 0;
...
PHP_FUNCTION(apn_payload_set_badge) {
    ...
    PHP_APN_PAYLOAD_FETCH_RESOURCE(payload, Z_RESVAL_P(res), rsrc_type, RETURN_FALSE);
    if (APN_ERROR == apn_payload_set_badge(payload, (int32_t) badge)) {
        error = apn_strerror(errno);
        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set badge: %s (%d)", error, errno);
        free(error);
        RETURN_FALSE
    }
    RETURN_TRUE
...
PHP_FUNCTION(apn_payload_set_body) {
    ...
    if (APN_ERROR == apn_payload_set_body(payload, value)) {
        error = apn_strerror(errno);
        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set alert message: %s (%d)", error, errno);
        free(error);
        RETURN_FALSE
    }
    RETURN_TRUE
...
PHP_FUNCTION(apn_payload_set_sound) {
    ...
    if (APN_ERROR == apn_payload_set_sound(payload, value)) {
        error = apn_strerror(errno);
        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set sound: %s (%d)", error, errno);
        free(error);
        RETURN_FALSE
    }
    RETURN_TRUE
...
PHP_FUNCTION(apn_payload_set_localized_key) {
    ...
    if (APN_ERROR == apn_payload_set_localized_key(payload, value, args, args_count)) {
        error = apn_strerror(errno);
        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set localized key: %s (%d)", error, errno);
        free(error);
        RETURN_FALSE
    }
    RETURN_TRUE
...
PHP_FUNCTION(apn_payload_set_launch_image) {
    ...
    if (APN_ERROR == apn_payload_set_launch_image(payload, value)) {
        error = apn_strerror(errno);
        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set launch image file name: %s (%d)", error, errno);
        free(error);
        RETURN_FALSE
    }
    RETURN_TRUE
...
PHP_FUNCTION(apn_payload_set_localized_action_key) {
    ...
    if (APN_ERROR == apn_payload_set_localized_action_key(payload, value)) {
        error = apn_strerror(errno);
        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set action key: %s (%d)", error, errno);
        free(error);
        RETURN_FALSE
    }
    RETURN_TRUE
...
PHP_FUNCTION(apn_payload_add_custom_property) {
    ...
    switch (Z_TYPE_P(value)) {
        case IS_BOOL:
            ret = apn_payload_add_custom_property_bool(payload, key, Z_BVAL_P(value));
            break;
        case IS_LONG:
            ret = apn_payload_add_custom_property_integer(payload, key, Z_LVAL_P(value));
            break;
        case IS_DOUBLE:
            ret = apn_payload_add_custom_property_double(payload, key, Z_DVAL_P(value));
            break;
        case IS_STRING:
            ret = apn_payload_add_custom_property_string(payload, key, Z_STRVAL_P(value));
            break;
        case IS_NULL:
            ret = apn_payload_add_custom_property_null(payload, key);
            break;
        default:
    ...
    if (APN_ERROR == ret) {
        error = apn_strerror(errno);
        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set custom property: %s (%d)", error, errno);
        free(error);
        RETURN_FALSE
    }
    RETURN_TRUE
...
PHP_FUNCTION(apn_payload_set_expiry) {
    ...
    PHP_APN_PAYLOAD_FETCH_RESOURCE(payload, Z_RESVAL_P(res), rsrc_type, RETURN_FALSE);
    apn_payload_set_expiry(payload, expiry);
    RETURN_TRUE
}
...
PHP_FUNCTION(apn_payload_set_priority) {
    ...
    if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "rl", &res, &priority)) {
        return;
    }

    PHP_APN_PAYLOAD_FETCH_RESOURCE(payload, Z_RESVAL_P(res), rsrc_type, RETURN_FALSE);
    apn_payload_set_priority(payload, priority);
    RETURN_TRUE
}
...
PHP_FUNCTION(apn_payload_set_category) {
    ...
    if (APN_ERROR == apn_payload_set_category(payload, value)) {
        error = apn_strerror(errno);
        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set category: %s (%d)", error, errno);
        free(error);
        RETURN_FALSE
    }
    RETURN_TRUE
...
PHP_FUNCTION(apn_payload_set_content_available) {
    ...
    PHP_APN_PAYLOAD_FETCH_RESOURCE(payload, Z_RESVAL_P(res), rsrc_type, RETURN_FALSE);
    apn_payload_set_content_available(payload, content_available);
    RETURN_TRUE
}
...
PHP_FUNCTION(apn_payload_set_array) {
            ...
            switch (flag) {
                case APN_PAYLOAD_OPTION_EXPIRY:
                    if (IS_LONG != Z_TYPE_PP(array_item)) {
                        ...
                        apn_payload_set_expiry(payload, (uint32_t) Z_LVAL_PP(array_item));
                    }
                    break;
                case APN_PAYLOAD_OPTION_BADGE:
                    if (IS_LONG != Z_TYPE_PP(array_item)) {
                        php_error_docref(NULL TSRMLS_CC, E_WARNING, "incorrect value type of `%s'. Expected numeric, got %s", array_strkey, __php_apn_var_type(*array_item));
                        RETURN_FALSE
                    } else if (APN_ERROR == apn_payload_set_badge(payload, (int32_t) Z_LVAL_PP(array_item))) {
                        error = apn_strerror(errno);
                        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set badge: %s (%d)", error, errno);
                        free(error);
                        RETURN_FALSE
                ...
                case APN_PAYLOAD_OPTION_CONTENT_AVAILABLE:
                    if (IS_BOOL != Z_TYPE_PP(array_item)) {
                        ...
                        apn_payload_set_content_available(payload, (uint8_t) Z_BVAL_PP(array_item));
                    }
                    break;
                case APN_PAYLOAD_OPTION_LAUNCH_IMAGE:
                    PHP_APN_OPTION_CHECK_STRING(array_strkey, array_item, continue);
                    if (APN_ERROR == apn_payload_set_launch_image(payload, Z_STRVAL_PP(array_item))) {
                        error = apn_strerror(errno);
                        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set launch image file name: %s (%d)", error, errno);
                        free(error);
                        RETURN_FALSE
                ...
                case APN_PAYLOAD_OPTION_LOC_ACTION_KEY:
                    PHP_APN_OPTION_CHECK_STRING(array_strkey, array_item, continue);
                    if (APN_ERROR == apn_payload_set_localized_action_key(payload, Z_STRVAL_PP(array_item))) {
                        error = apn_strerror(errno);
                        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set localized action key: %s (%d)", error, errno);
                        free(error);
                        RETURN_FALSE
                    }
                    break;
                case APN_PAYLOAD_OPTION_SOUND:
                    PHP_APN_OPTION_CHECK_STRING(array_strkey, array_item, continue);
                    if (APN_ERROR == apn_payload_set_sound(payload, Z_STRVAL_PP(array_item))) {
                        error = apn_strerror(errno);
                        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set sound file name: %s (%d)", error, errno);
                        free(error);
                        RETURN_FALSE
                    }
                    break;
                case APN_PAYLOAD_OPTION_BODY:
                    PHP_APN_OPTION_CHECK_STRING(array_strkey, array_item, continue);
                    if (APN_ERROR == apn_payload_set_body(payload, Z_STRVAL_PP(array_item))) {
                        error = apn_strerror(errno);
                        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set body: %s (%d)", error, errno);
                        free(error);
                        RETURN_FALSE
                    }
                    break;
                ...
                case APN_PAYLOAD_OPTION_TOKENS:
                    ...
                    break;
                case APN_PAYLOAD_OPTION_CATEGORY:
                    PHP_APN_OPTION_CHECK_STRING(array_strkey, array_item, continue);
                    if (APN_ERROR == apn_payload_set_category(payload, Z_STRVAL_PP(array_item))) {
                        error = apn_strerror(errno);
                        php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set category: %s (%d)", error, errno);
                        free(error);
                        RETURN_FALSE
                    ...
                    break;
                case APN_PAYLOAD_OPTION_PRIORITY:
                    if (IS_LONG != Z_TYPE_PP(array_item)) {
                        php_error_docref(NULL TSRMLS_CC, E_WARNING, "incorrect value type of `%s'. Expected numeric, got %s", array_strkey, __php_apn_var_type(*array_item));
                        RETURN_FALSE
                    } else {
                        apn_payload_set_priority(payload, (apn_notification_priority) Z_LVAL_PP(array_item));
                    }
                    break;
                default:
                    php_error_docref(NULL TSRMLS_CC, E_WARNING, "unknown option: `%s'", array_strkey);
    ...
    if (loc_key && loc_key_length > 0) {
        if (APN_ERROR == apn_payload_set_localized_key(payload, loc_key, args, args_count)) {
            error = apn_strerror(errno);
            php_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to set localized key: %s (%d)", error, errno);
            free(error);
            RETURN_FALSE
...
PHP_FUNCTION(apn_feedback_connect) {
    ...
    if (APN_ERROR == apn_feedback_connect(apn_ctx)) {
        error = apn_strerror(errno);
        ZVAL_STRING(ref_error, error, 1);
        ZVAL_LONG(ref_errcode, errno);
        free(error);
...
PHP_FUNCTION(apn_feedback) {
    ...
    if (APN_ERROR == apn_feedback(apn_ctx, &tokens, &tokens_count)) {
        error = apn_strerror(errno);
        ZVAL_STRING(ref_error, error, 1);
        ZVAL_LONG(ref_errcode, errno);
        free(error);
